...
class InteractionCollector:
    ...
    def __init__(
        ...
        self._control_boundaries = deque(maxlen=100)
    ...
    def aggregate_between(self, start_time, end_time):
        ...
        with self._lock:
            ...
            boundaries = [
                boundary for boundary in self._control_boundaries
                if start_time <= boundary <= end_time
            ]
        excluded = set()
        for boundary in boundaries:
            candidates = [
                (abs((event.timestamp - boundary).total_seconds()), index)
                for index, event in enumerate(matched)
                if event.interaction_type == InteractionType.KEYBOARD_ACTIVITY
            ]
            if candidates:
                distance, index = min(candidates)
                if distance <= 0.5:
                    excluded.add(index)
        matched = [event for index, event in enumerate(matched) if index not in excluded]
    ...
    def mark_control_boundary(self, timestamp=None):
        """Exclude the nearest keyboard callback generated by a calibration hotkey."""
        boundary = timestamp or self.clock()
        with self._lock:
            self._control_boundaries.append(boundary)
        return boundary
